...
# 엑셀읽기
workbook = openpyxl.load_workbook(r'C:\Users\스타트코딩\Desktop\main\python\주식자동화\연습\kospi.xlsx')
...
try:
    for i in range(2, 5000):
        if(type(sheet[f'B{i}'].value) != str):
            break;
        
        stock_code_list.append(sheet[f'A{i}'].value)
except IndexError:
    pass
...
for idx, stock_code in enumerate(stock_code_list, 1) : 
    ...
    sheet[f'J{idx + 1}'] = short_minus_long
    sheet[f'K{idx + 1}'] = long_minus_short
    ...
    # 현재가
    sheet[f'C{idx + 1}'] = current_price

    # 단기매수한계저가
    sheet[f'H{idx + 1}'] = short_mesuhange_gega

    # 장기매수한계저가
    sheet[f'I{idx + 1}'] = long_mesuhange_gega
